...
Usage:
    ...
    python test_policy.py --checkpoint runs/PiperEnv__ppo_rgb__1__<timestamp>/best_ckpt.pt --episodes 5
    ...
    python test_policy.py --save-video --video-camera wrist_cam  # Use wrist camera for video
"""
...
def test_policy(checkpoint_path, num_episodes=3, deterministic=True, max_steps=128, render=True, save_video=False, video_path=None, video_camera="rgb"):
    """
    Test a trained policy by loading the checkpoint and running episodes.
    Handles RGB + wrist_cam + state observation format used in training.
    Optionally saves video of the test process.
    
    Args:
        video_camera: Which camera to use for video recording ("rgb" or "wrist_cam")
    """
    ...
    if save_video:
        print(f"Video recording: enabled, saving to {video_path} (using {video_camera} camera)")
    print("-" * 60)
    ...
    if save_video:
        try:
            # Get camera observation to determine video dimensions
            temp_obs, _ = env.reset()
            camera_shape = temp_obs[video_camera].shape
            height, width = camera_shape[0], camera_shape[1]
            
            if IMAGEIO_AVAILABLE:
                # Use imageio for video recording
                print(f"✓ Video recording initialized (imageio): {width}x{height} using {video_camera} camera")
            else:
    ...
    obs, _ = env.reset()
    print(f"Observation format: RGB shape {obs['rgb'].shape}, Wrist cam shape {obs['wrist_cam'].shape}, State shape {obs['state'].shape}")
    ...
    sample_obs = {
        ...
        'wrist_cam': torch.tensor(obs['wrist_cam']).unsqueeze(0),
    ...
    for episode in range(num_episodes):
        ...
        for step in range(max_steps):
            ...
            if save_video and IMAGEIO_AVAILABLE:
                camera_frame = next_obs[video_camera]
                # Store frames for imageio
                video_frames.append(camera_frame)
...
def main():
    ...
    parser.add_argument("--video-camera", type=str, default="rgb", choices=["rgb", "wrist_cam"],
                        help="Which camera to use for video recording")
