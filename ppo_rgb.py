...
class NatureCNN(nn.Module):
    def __init__(self, sample_obs):
        ...
        # Create separate CNN for rgb (3rd person camera)
        rgb_cnn = nn.Sequential(
            nn.Conv2d(
        ...
        # Create separate CNN for wrist_cam (same architecture)
        ...
        # Figure out dimensions after flattening for both CNNs
        with torch.no_grad():
            n_flatten_rgb = rgb_cnn(sample_obs["rgb"].float().permute(0,3,1,2).cpu()).shape[1]
            rgb_fc = nn.Sequential(nn.Linear(n_flatten_rgb, feature_size), nn.ReLU())
            
            n_flatten_wrist = wrist_cnn(sample_obs["wrist_cam"].float().permute(0,3,1,2).cpu()).shape[1]
            wrist_fc = nn.Sequential(nn.Linear(n_flatten_wrist, feature_size), nn.ReLU())
        
        extractors["rgb"] = nn.Sequential(rgb_cnn, rgb_fc)
        extractors["wrist_cam"] = nn.Sequential(wrist_cnn, wrist_fc)
        self.out_features += feature_size * 2  # Two camera features
...
def train(args: PPOArgs):
    ...
    best_eval_reward = float('-inf')  # Track best evaluation reward for checkpoint saving
        ...
        if iteration % args.eval_freq == 1:
            ...
            if eval_metrics and logger is not None:
                if "r" in eval_metrics:
                    ...
                    current_eval_reward = np.mean(episode_rewards)
                    logger.add_scalar("eval/episode_reward_mean", current_eval_reward, global_step)
                    ...
                    print(f"eval_reward_mean={current_eval_reward:.3f}")
                    
                    # Check if this is a new best evaluation reward
                    if current_eval_reward > best_eval_reward:
                        best_eval_reward = current_eval_reward
                        print(f"New best evaluation reward: {best_eval_reward:.3f}")
                        
                        # Save checkpoint for new best
                        if args.save_model:
                            model_path = f"runs/{run_name}/best_ckpt.pt"
                            torch.save(agent.state_dict(), model_path)
                            print(f"Best model saved to {model_path}")
                    
                    # Always log the best eval reward for tracking
                    logger.add_scalar("eval/best_episode_reward", best_eval_reward, global_step)
    ...
    if args.save_model and not args.evaluate:
        ...
        if best_eval_reward != float('-inf'):
            print(f"Best evaluation reward achieved: {best_eval_reward:.3f}")
        else:
